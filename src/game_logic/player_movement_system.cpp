...
void PlayerMovementSystem::update(
  ...
  if (!horizontalMovementWanted) {
    ...
    if (state.mState == PlayerState::Standing) {
      const auto noDirectionChange =
        false;
        //(movingLeft && oldOrientation == Orientation::Left) ||
        //(!movingLeft && oldOrientation == Orientation::Right);

      if (mWalkRequestedLastFrame || noDirectionChange) {
        state.mState = PlayerState::Walking;
      }
    }
